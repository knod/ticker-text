...
//         	return ( root.TTUI = displayFactory( jquery, nouiCSS ) );
//         });
...
//     } else {
//         // Browser globals
//         root.TTUI = displayFactory( root.jQuery, root.nouiCSS );
//     }
...
(function (root, uiFactory) {  // root is usually `window`
    if (typeof define === 'function' && define.amd) {
        // AMD. Register as an anonymous module.
        define( [ 'jquery' ], function ( jquery ) { return ( root.TTUI = uiFactory( jquery ) ); });
    } else if (typeof module === 'object' && module.exports) {
        // Node. Does not work with strict CommonJS, but only CommonJS-like
        // environments that support module.exports, like Node.
	...
	*/
	...
	<div id="__tt_below_bar" class="__tt-main-section __tt-hidden"></div>\n\
		...
		ttui._addNodes = function () {
			...
			tickerText = ttui._tickerTextNode = $(htmlStr)[0];
			$(tickerText).appendTo( iDoc.body );
			console.log( "in body?:", tickerText );

			var styles1 	= iDoc.createElement("link");
			styles1.href 	= browser.runtime.getURL( filepaths.ui.core );
